...
def gather_data():
    with open('/tmp/workers.pkl','rb') as f:  
        ...
        #     "data/seq/", 
        #     datetime.now().strftime('%Y-%m-%d_%H-%M-%S'))
        mydir = "throughput/direct-2/seq_data/"
        # print(mydir)
        try:
            os.makedirs(mydir)
        except OSError as e:
        ...
        for worker in workers:
            ...
            # remoteCmd = 'scp -o StrictHostKeyChecking=no {}:/tmp/trace_tcp_probe.txt {}'.format(worker['host'], mydir)
