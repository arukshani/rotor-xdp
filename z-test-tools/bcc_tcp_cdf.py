...
int trace_tcp_rcv(struct pt_regs *ctx, struct sock *sk, struct sk_buff *skb)
{
    ...
    struct srtt_list data_rtt = {};
    data_rtt.srtt_val = srtt;
    srtt_events.perf_submit(ctx, &data_rtt, sizeof(data_rtt));
...
srtt_log =[]
# i = 0
def print_srtt_events(cpu, data, size):
    # global i
    global srtt_log
    event = b["srtt_events"].event(data)
    # print("\t%d" % event.srtt_val)
    srtt_log.append(event.srtt_val)
    # i = i + 1
...
b["srtt_events"].open_perf_buffer(print_srtt_events)
while 1:
    try:
        b.perf_buffer_poll()
    except KeyboardInterrupt:
        # print(srtt_log)
        with open("/tmp/srtt_values.txt", "w") as txt_file:
            txt_file.write(str(srtt_log))
        exit()
