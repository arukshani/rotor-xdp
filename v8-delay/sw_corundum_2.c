...
// #if DEBUG_PAUSE_Q == 1
	...
	for (z = 0; z < send_time_index; z++)
	{
		...
		// int seq = 0;
		// int is_fwd_path = 0;
		fprintf(fpt, "%d,%d,%ld,%ld\n", send_seq[z], send_slot[z], send_timestamp_arr[z].tv_sec, send_timestamp_arr[z].tv_nsec);
	}
	// int z;
	for (z = 0; z < recv_time_index; z++)
	{
		// unsigned long now_ns = get_nsec(&timestamp_arr[z]);
		// int seq = 0;
		// int is_fwd_path = 0;
		fprintf(fpt, "%d,%d,%ld,%ld\n", recv_seq[z], recv_slot[z], recv_timestamp_arr[z].tv_sec, recv_timestamp_arr[z].tv_nsec);
	}
	fclose(fpt);
// #endif
